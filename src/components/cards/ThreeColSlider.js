...
const ControlButton = styled(PrimaryButtonBase)`
  ${tw`mt-4 sm:mt-0 first:ml-0 ml-6 rounded-full p-2 bg-yellow-500`}
  svg {
    ${tw`w-6 h-6`}
  }
`;
    ...
    <Container>
      <Content>
        <HeadingWithControl>
          ...
          <Controls>
            <PrevButton bgColor={bgColor} onClick={sliderRef?.slickPrev}>
              <ChevronLeftIcon />
            </PrevButton>
            <NextButton bgColor={bgColor} onClick={sliderRef?.slickNext}>
              <ChevronRightIcon />
            </NextButton>
          </Controls>
        </HeadingWithControl>
        <CardSlider ref={setSliderRef} {...sliderSettings}>
          {cards.map((card, index) => (
            <Card key={index}>
              ...
              <PrimaryButton bgColor={bgColor}>Book Now</PrimaryButton>
            </Card>
          ))}
        </CardSlider>
      </Content>
